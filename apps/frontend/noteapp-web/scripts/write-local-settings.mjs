...
const inputPath = process.env.NOTEAPP_SETTINGS_INPUT_JSON
  ? resolve(process.env.NOTEAPP_SETTINGS_INPUT_JSON)
  : '';
const outputPath = process.env.NOTEAPP_SETTINGS_SNAPSHOT_OUTPUT
  ? resolve(process.env.NOTEAPP_SETTINGS_SNAPSHOT_OUTPUT)
  : resolve(appRoot, 'public', 'fixtures', 'local-settings-snapshot.json');
...
function printHelp() {
  console.log(`write-local-settings

Write .noteapp/settings.json through the desktop sync CLI and refresh the settings snapshot.
...
Required environment:
  ...
  NOTEAPP_SETTINGS_INPUT_JSON   JSON file containing local settings
...
function buildCommand() {
  ...
  commandArgs.push('write-local-settings', '--input-json', requireEnv('NOTEAPP_SETTINGS_INPUT_JSON'));
