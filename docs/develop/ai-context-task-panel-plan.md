...
把“当前笔记、文件夹、搜索结果、用户多选文档”统一收口为一个能力：把选取内容加入 AI 上下文，然后由用户输入任意指令生成结果。总结、写报告、提炼行动项、改写、对比、找风险都只是同一个上下文任务上的不同指令。

核心原则：

1. 用户不是只和 AI 聊天，而是在明确上下文里发起任务。
2. 所有任务都必须基于可追溯来源生成，输出保留 citations / sources。
3. 输出先预览，再由用户决定插入当前笔记或保存为新知识页。
4. 第一版优先支持真实高频场景：选中文件夹或多个文档后，将这些文档内容加入 AI 上下文，再让用户自由输入任务。
...
5. 每个文件最多取前 4000 字。
6. 总上下文最多 30000 字。
7. 超出限制时在结果顶部提示“仅总结前 N 个文件 / 部分内容已截断”。
...
1. `folder + preview`：选中文件夹，把文件夹 Markdown 加入 AI 上下文，用户输入任意指令后生成结果。
2. `selected_files + preview`：多选几个 Markdown，把这些文档加入 AI 上下文，用户输入任意指令后生成结果。
3. 输出带来源。
4. UI 先只做预览，不急着插入/保存。
